...
- Sharing through T3 Connect needs no tailnet. Authorize the worktree once with `node apps/server/src/bin.ts connect login --base-dir .t3`, then run `vp run dev --share --share-via=t3-connect --home-dir .t3` in the background. Wait for the managed `pairingUrl:` line and paste that full URL (token included) in your reply. Plain `--share` remains the Tailscale path.
