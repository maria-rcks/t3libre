...
const CloudCliTokenManagerLive = Layer.unwrap(
  Effect.gen(function* () {
    const config = yield* ServerConfig.ServerConfig;
    const path = yield* Path.Path;
    const secretsDir = config.connectDevShareAuthorizationBaseDir
      ? path.join(config.connectDevShareAuthorizationBaseDir, "userdata", "secrets")
      : config.secretsDir;
    const secretStoreConfig = ServerConfig.make({ ...config, secretsDir });
    const authorizationSecretStoreLayer = ServerSecretStore.layer.pipe(
      Layer.provide(ServerConfig.layer(secretStoreConfig)),
    );
    return CloudCliTokenManager.layer.pipe(
      Layer.provide(authorizationSecretStoreLayer),
...
const RuntimeCoreDependenciesLive = ReactorLayerLive.pipe(
  ...
  Layer.provideMerge(Layer.mergeAll(CloudCliTokenManagerLive, CloudManagedEndpointRuntimeLive)),
);
          ...
          Effect.gen(function* () {
            ...
            const reconcileCloudLink = Effect.gen(function* () {
              ...
              return yield* reconcileDesiredCloudLink(`http://127.0.0.1:${address.port}`, {
                ...
                ...(connectDevShare && config.connectDevShareAuthorizationBaseDir
                  ? { authorizationBaseDir: config.connectDevShareAuthorizationBaseDir }
                  : {}),
              });
            });
