...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const readWorkingTreeReviewFile = Effect.fn("readWorkingTreeReviewFile")(function* (
    ...
    const relativeRequestedPath = path.relative(repositoryRoot, requestedPath);
    if (
    ...
    const [realRepositoryRoot, realTarget] = yield* Effect.all([
      ...
      Effect.mapError((cause) =>
        reviewDiffFileError(input, `Could not resolve diff file '${input.newPath}'.`, cause),
      ),
    );
    const relativeRealPath = path.relative(realRepositoryRoot, realTarget);
    if (
      relativeRealPath === ".." ||
    ...
    const info = yield* fileSystem
      ...
      .pipe(
        Effect.mapError((cause) =>
          reviewDiffFileError(input, `Could not inspect diff file '${input.newPath}'.`, cause),
        ),
      );
    if (info.type !== "File") {
      return yield* reviewDiffFileError(input, `Diff path '${input.newPath}' is not a file.`);
    }
    ...
    const bytes = yield* fileSystem
      ...
      .pipe(
        Effect.mapError((cause) =>
          reviewDiffFileError(input, `Could not read diff file '${input.newPath}'.`, cause),
        ),
      );
    if (bytes.includes(0)) {
      return yield* reviewDiffFileError(input, `Cannot expand binary file '${input.newPath}'.`);
    }
