  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const snapshot = yield* makeManagedServerProvider<ProviderSnapshotSettings<DevinSettings>>({
        ...
        checkProvider,
        // Discovery creates an ACP session; refresh on demand rather than waking the CLI periodically.
      ...
      const provider = yield* makeDevinRuntimeMetadata(snapshot);
      const adapter = yield* makeDevinAdapter(effectiveConfig, {
        environment: processEnv,
        ...(eventLoggers.native ? { nativeEventLogger: eventLoggers.native } : {}),
        instanceId,
        onSessionStarted: (started) =>
          provider.onConfigOptionsUpdated(started.sessionSetupResult.configOptions ?? []),
