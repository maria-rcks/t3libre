...
for (const enabled of [false, true]) {
  it.effect(`persists shutdown recovery before stopping providers when enabled=${enabled}`, () =>
    Effect.gen(function* () {
      ...
      assert.propertyVal(markers[0], "activeTurnId", turnId);
      if (enabled) {
        ...
        assert.notProperty(markers[0], "continueAfterServerUpdate");
        assert.notProperty(binding.value.runtimePayload, "continueAfterServerUpdate");
      }
