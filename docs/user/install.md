...
| OpenCode 2 Preview | [OpenCode 2](https://opencode.ai/v2)                  | `opencode2`    | `opencode2 auth login` |

Codex and Claude are on by default. OpenCode 2 is enabled automatically when T3 Code detects the
preview CLI, and a one-time notice links to its Settings card. Cursor, Grok Build, and stable
OpenCode are off by default; turn them on in **Settings** → the provider's card when you want to
use them.
